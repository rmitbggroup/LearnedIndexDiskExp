...
class AlexModelNode : public AlexNode<T, P> {
 public:
  ...
  PhyscialAddr *childrenAddrs = nullptr;
  int *is_leaf = nullptr;
  ...
  inline AlexNode<T, P>* get_child_node(const T& key) {
   ...
   inline PhyscialAddr get_child_node(T key, int *pos) {
    int bucketID = this->model_.predict(key);
    bucketID = std::min<int>(std::max<int>(bucketID, 0), num_children_ - 1);
    *pos = bucketID;
    return childrenAddrs[bucketID];
  }
  ...
  int expand_disk(int log2_expansion_factor) {
    ...
    auto new_children_addr = new PhyscialAddr[num_new_children];
    int cur = 0;
    //FIXME: trick, we can store the duplication_factor_ into the PhyscialAddr part...
    // but, if update, we still need to update that ...
    while (cur < num_children_) {
      PhyscialAddr cur_child = childrenAddrs[cur];
      ...
      for (int i = expansion_factor * cur;
        ...
        new_children_addr[i] = cur_child;
      }
      cur += cur_child_repeats;
    }
    // pointer_allocator().deallocate(children_, num_children_);
    delete []childrenAddrs;
    childrenAddrs = new_children_addr;
  ...
  // 1 is LEAF_DISK
  int expand(int log2_expansion_factor, int mode) {
    assert(log2_expansion_factor >= 0);
    int expansion_factor = 1 << log2_expansion_factor;
    int num_new_children = num_children_ * expansion_factor;
    auto new_children = new (pointer_allocator().allocate(num_new_children))
        AlexNode<T, P>*[num_new_children];
    auto new_is_leaf = new int[num_new_children];
    auto new_child_address = new PhyscialAddr[num_new_children];
    int cur = 0;
    while (cur < num_children_) {
      AlexNode<T, P>* cur_child = children_[cur];
      int cur_child_repeats;// = 1 << (mode == 1 ? childrenAddrs[cur].duplication_factor_: cur_child->duplication_factor_);
      if (mode == 1 && is_leaf[cur] == 0 || mode != 1) {
          cur_child_repeats = 1 << cur_child->duplication_factor_;
          cur_child->duplication_factor_ += log2_expansion_factor;
      } else if (mode == 1 && is_leaf[cur] == 1){
          cur_child_repeats = 1 << childrenAddrs[cur].duplication_factor_;
          childrenAddrs[cur].duplication_factor_ += log2_expansion_factor;
      }
      for (int i = expansion_factor * cur;
           i < expansion_factor * (cur + cur_child_repeats); i++) {
        new_children[i] = cur_child;
        new_is_leaf[i] = is_leaf[cur];
        new_child_address[i] = childrenAddrs[cur];
      }
      cur += cur_child_repeats;
    }
    pointer_allocator().deallocate(children_, num_children_);
    ...
    num_children_ = num_new_children;
    is_leaf = new_is_leaf;
    childrenAddrs = new_child_address;
 ...
 public:
  ...
  int num_keys_in_range(int left, int right) const {
    ...
    if (left_bitmap_idx == right_bitmap_idx) {
      ...
      #if M1_Chip
      num_keys += count_ones(bitmap_data);
      #else
      ...
      #if M1_Chip
      num_keys += count_ones(left_bitmap_data);
      #else
      ...
      if (right_bitmap_idx != bitmap_size_) {
        ...
        #if M1_Chip
        num_keys += count_ones(right_bitmap_data);
        #else
    ...
    if (bitmap_[bitmap_pos] == static_cast<uint64_t>(-1) ||
      ...
      while (bitmap_distance <= max_bidirectional_bitmap_offset) {
        ...
        if (left_bitmap_data != static_cast<uint64_t>(-1) &&
          ...
          int left_gap_pos = ((bitmap_pos - bitmap_distance + 1) << 6) -
                             static_cast<int>(_nlz(~left_bitmap_data)) -
                             1;
          int right_gap_pos = ((bitmap_pos + bitmap_distance) << 6) +
                              static_cast<int>(_tzc(~right_bitmap_data));
          #else
          int left_gap_pos = ((bitmap_pos - bitmap_distance + 1) << 6) -
          ...
          if (bit_pos > 32 && bitmap_pos + bitmap_distance + 1 < bitmap_size_ &&
            ...
            int right_gap_pos =
                ...
                static_cast<int>(
                    _tzc(~bitmap_[bitmap_pos + bitmap_distance + 1]));
            #else
            int right_gap_pos =
                ((bitmap_pos + bitmap_distance + 1) << 6) + 
                static_cast<int>(
          ...
          if (right_gap_pos < data_capacity_) {
            ...
            if (bit_pos < 32 && bitmap_pos - bitmap_distance > 0 &&
              ...
              int left_gap_pos =
                  ...
                  static_cast<int>(
                      _nlz(~bitmap_[bitmap_pos - bitmap_distance - 1])) -
                  1;
              #else
              int left_gap_pos =
                  ((bitmap_pos - bitmap_distance) << 6) -
      ...
      if (max_left_bitmap_offset > max_right_bitmap_offset) {
        for (int i = bitmap_pos - bitmap_distance; i >= left_bitmap_pos; i--) {
          if (bitmap_[i] != static_cast<uint64_t>(-1)) {
            #if M1_Chip
            return ((i + 1) << 6) - static_cast<int>(_nlz(~bitmap_[i])) -
                   1;
            #else
        ...
        for (int i = bitmap_pos + bitmap_distance; i <= right_bitmap_pos; i++) {
          if (bitmap_[i] != static_cast<uint64_t>(-1)) {
            #if M1_Chip
            int right_gap_pos =
                (i << 6) + static_cast<int>(_tzc(~bitmap_[i]));
            #else
            int right_gap_pos =
      ...
      if (bitmap_right_gaps != 0) {
        ...
        closest_right_gap_distance =
            static_cast<int>(_tzc(bitmap_right_gaps)) - bit_pos;
        #else
        closest_right_gap_distance =
        ...
        #if M1_Chip
        closest_right_gap_distance =
            64 + static_cast<int>(_tzc(~bitmap_[bitmap_pos + 1])) -
            bit_pos;
        #else
        closest_right_gap_distance =
      ...
      if (bitmap_left_gaps != 0) {
        ...
        closest_left_gap_distance =
            bit_pos - (63 - static_cast<int>(_nlz(bitmap_left_gaps)));
        #else
        closest_left_gap_distance =
        ...
        #if M1_Chip
        closest_left_gap_distance =
            bit_pos + static_cast<int>(_nlz(~bitmap_[bitmap_pos - 1])) +
            1;
        #else
        closest_left_gap_distance =
