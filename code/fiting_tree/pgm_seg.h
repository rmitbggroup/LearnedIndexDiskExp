// This file is the copy from the coresponding part of PGM-index <https://github.com/gvinciguerra/PGM-index>.
...
// template<typename T>
// using LargeSigned = typename std::conditional_t<std::is_floating_point<T>,
//                                                 long double,
//                                                 std::conditional_t<(sizeof(T) < 8), int64_t, __int128>>;

using LargeSignedK = __int128;
using LargeSignedV = __int128;
const bool IsInteger = true;

template<typename X, typename Y>
class OptimalPiecewiseLinearModel {
private:
    using SX = LargeSignedK; //<X>;
    using SY = LargeSignedV; //<Y>;
...
public:
    ...
    CanonicalSegment get_segment(SY start_index, SY end_index) {
        if (points_in_hull == 1)
            return CanonicalSegment(rectangle[0], rectangle[1], first_x, start_index, end_index);
        return CanonicalSegment(rectangle, first_x, start_index, end_index);
    }
...
class OptimalPiecewiseLinearModel<X, Y>::CanonicalSegment {
    ...
    CanonicalSegment(const Point &p0, const Point &p1, X first, SY start_index, SY end_index) : rectangle{p0, p1, p0, p1}, first(first), start_index(start_index), end_index(end_index) {};

    CanonicalSegment(const Point (&rectangle)[4], X first, SY start_index, SY end_index)
        : rectangle{rectangle[0], rectangle[1], rectangle[2], rectangle[3]}, first(first), start_index(start_index), end_index(end_index) {};
...
public:
    ...
    SY get_start_index() const {return start_index; }
    SY get_end_index() const {return end_index; }
    ...
    std::pair<long double, SY> get_floating_point_segment(const X &origin) const {
        ...
        auto i = get_intersection();
        auto i_x = i.first;
        auto i_y = i.second;
        // auto[min_slope, max_slope] = get_slope_range();
        auto _slope = get_slope_range();
        auto min_slope = _slope.first;
        auto max_slope = _slope.second;
...
size_t make_segmentation(size_t n, size_t epsilon, Fin in, Fout out) {
    ...
    for (size_t i = 1; i < n; ++i) {
        ...
        if (!opt.add_point(p.first, p.second)) {
            end_index = i - 1;
            out(opt.get_segment(start_index, end_index));
            start_index = i;
            opt.add_point(p.first, p.second);
            ++c;
        }
    ...
    out(opt.get_segment(start_index, n - 1));
    return ++c;
}

template<typename Fin, typename Fout>
size_t make_segmentation_par(size_t n, size_t epsilon, Fin in, Fout out) {
    auto parallelism = 1; //std::min(std::min(omp_get_num_procs(), omp_get_max_threads()), 20);
